...
-- Define a function to open the specified file
...
function OPEN_IN_OPERA_GX(current_file)
	local opera_invoke = "cmd.exe /C start \"\" \"$(wslpath -w /mnt/c/Users/Marko/AppData/Local/Programs/Opera\\ GX/launcher.exe)\" $(wslpath -w " .. current_file ..")"
...
vim.keymap.set("n", '<A-n>e', '<cmd>lua vim.diagnostic.goto_next( {severity = vim.diagnostic.severity.ERROR})<CR>', {silent = true})
vim.keymap.set("n", '<A-m>e', '<cmd>lua vim.diagnostic.goto_prev( {severity = vim.diagnostic.severity.ERROR})<CR>', {silent = true})

vim.keymap.set("n", '<A-n>w', '<cmd> lua vim.diagnostic.goto_next( {severity = vim.diagnostic.severity.WARN})<CR>', {silent = true})
vim.keymap.set("n", '<A-m>w', '<cmd> lua vim.diagnostic.goto_prev( {severity = vim.diagnostic.severity.WARN})<CR>', {silent = true})
